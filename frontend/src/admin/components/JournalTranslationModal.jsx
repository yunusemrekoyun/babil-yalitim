...
export default function JournalTranslationModal({
  open,
  journalId,
  journalTitle,
  onClose,
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        setError("");
        const { data } = await api.get(`/journals/${journalId}/translations`);
        if (cancelled) return;
        ...
        setError(
          getAdminFeedbackMessage(
            fetchError,
            "Haber çevirisi bilgileri alınamadı."
          )
        );
      } finally {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      setSaving(true);
      setError("");
      const { data } = await api.put(`/journals/${journalId}/translations`, {
        ...
        <div className="flex items-start justify-between gap-4 border-b border-slate-200/70 px-6 py-5 dark:border-slate-800">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
              {journalTitle || "Seçili haber"} için İngilizce karşılıkları gir.
              Boş kalan alanlarda public tarafta Türkçe içerik gösterilir.
            </p>
          </div>
          <button
            type="button"
...
JournalTranslationModal.propTypes = {
